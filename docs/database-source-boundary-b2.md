...
建议采用渐进式收敛：**Prisma 负责表、字段、普通唯一约束、普通索引和关系；手写 SQL 负责视图、函数、触发器、检查约束、部分索引、表达式索引以及 PostgreSQL 特有能力。** 在方案确认前，本报告不修改任何数据库结构或迁移顺序。
...
1. **冻结漂移**：从确认日开始，新表和普通字段不再同时修改两侧；CI 检查新增 SQL 中是否出现未经豁免的 `CREATE TABLE` 或普通 `ADD COLUMN`。
2. **补全 Prisma 描述**：先把 16 张 SQL 独有表、3 个枚举和 11 个仍在使用的字段加入 Prisma，全部使用现有数据库名映射，不删除任何对象。
3. **解决真实冲突**：单独迁移 `audit_logs.actor/actor_id`，统一文章作者默认值，确认 UUID 与更新时间由数据库保护的范围。
4. **核对索引与约束**：普通索引归 Prisma；部分、表达式、GIN 和 CHECK 约束保留 SQL。先去重后再调整，不以索引名称直接判断。
5. **设立切换点**：历史 SQL 表迁移保持不可变，作为旧版本建库历史；切换点之后的新表结构只进入 Prisma migration。SQL 能力迁移在相关 Prisma migration 之后执行。
6. **重做空库验收顺序**：在隔离空库中验证“历史基线 -> Prisma 结构迁移 -> SQL 能力迁移 -> 可选回填”，并与生产结构做无损 diff。
7. **增加 CI 门禁**：每次数据库变更创建两个临时空库，执行完整迁移后检查 Prisma drift、SQL 能力对象存在性、重复索引和未分类表 DDL。
