...
const copy: Record<SiteLocale, DisclosureCopy> = {
  zh: {
    ...
    sections: [
      ...
      { title: "同意、第三方与保存期", body: "需要同意时，分析脚本只会在你选择接受后运行。网站可配置 Google Analytics 或 Google Tag Manager，Google 会按其政策处理收到的数据。GeoSub 的第一方原始分析事件默认最多保存 180 天，之后删除或仅保留汇总统计。" },
  ...
  "zh-tw": {
    ...
    sections: [
      ...
      { title: "同意、第三方與保存期間", body: "需要同意時，分析程式只會在你選擇接受後執行。網站可設定 Google Analytics 或 Google Tag Manager，Google 會依其政策處理收到的資料。GeoSub 的第一方原始分析事件預設最多保存 180 天，之後刪除或僅保留彙總統計。" },
  ...
  en: {
    ...
    sections: [
      ...
      { title: "Consent, providers, and retention", body: "Where consent is required, analytics run only after you accept. The site may be configured with Google Analytics or Google Tag Manager; Google processes received data under its own policies. GeoSub retains first-party raw analytics events for up to 180 days by default, then deletes them or keeps only aggregated statistics." },
  ...
  ja: {
    ...
    sections: [
      ...
      { title: "同意・外部サービス・保存期間", body: "同意が必要な場合、分析は同意後にのみ実行されます。Google Analytics または Google Tag Manager を設定する場合があり、Googleは受領したデータを同社の方針に従って処理します。GeoSubの生の分析イベントは原則180日以内に削除するか、集計情報のみを残します。" },
  ...
  ko: {
    ...
    sections: [
      ...
      { title: "동의, 외부 서비스 및 보관", body: "동의가 필요한 경우 분석은 동의한 뒤에만 실행됩니다. Google Analytics 또는 Google Tag Manager가 설정될 수 있으며 Google은 자체 정책에 따라 수신 데이터를 처리합니다. GeoSub의 원시 분석 이벤트는 기본적으로 최대 180일 보관한 뒤 삭제하거나 집계 통계만 남깁니다." },
  ...
  es: {
    ...
    sections: [
      ...
      { title: "Consentimiento, proveedores y conservación", body: "Cuando se requiere consentimiento, la analítica solo funciona después de aceptarla. El sitio puede usar Google Analytics o Google Tag Manager; Google trata los datos recibidos conforme a sus propias políticas. GeoSub conserva los eventos analíticos propios sin agregar durante un máximo predeterminado de 180 días y después los elimina o conserva solo estadísticas agregadas." },
  ...
  tr: {
    updated: "Son güncelleme: 14 Ağustos 2026",
    sections: [
      ...
      { title: "Onay, sağlayıcılar ve saklama", body: "Onayın gerektiği yerlerde analiz yalnızca kabulden sonra çalışır. Google Analytics veya Google Tag Manager yapılandırılabilir; Google aldığı verileri kendi politikalarına göre işler. GeoSub birinci taraf ham analiz olaylarını varsayılan olarak en fazla 180 gün saklar, ardından siler veya yalnızca toplu istatistikleri korur." },
  ...
  ar: {
    updated: "آخر تحديث: 14 أغسطس 2026",
    sections: [
      ...
      { title: "الموافقة ومقدمو الخدمة والاحتفاظ", body: "عندما تكون الموافقة مطلوبة، لا تعمل التحليلات إلا بعد قبولها. قد يستخدم الموقع Google Analytics أو Google Tag Manager، وتعالج Google البيانات وفق سياساتها. يحتفظ GeoSub بأحداث التحليل الأولية الخاصة به لمدة افتراضية لا تتجاوز 180 يوماً، ثم يحذفها أو يبقي الإحصاءات المجمعة فقط." },
  ...
  fr: {
    ...
    sections: [
      ...
      { title: "Consentement, prestataires et conservation", body: "Lorsque le consentement est requis, les outils d’analyse ne fonctionnent qu’après acceptation. Le site peut utiliser Google Analytics ou Google Tag Manager ; Google traite les données reçues selon ses propres règles. GeoSub conserve par défaut les événements d’analyse bruts de première partie pendant 180 jours au maximum, puis les supprime ou ne garde que des statistiques agrégées." },
  ...
  it: {
    ...
    sections: [
      ...
      { title: "Consenso, fornitori e conservazione", body: "Quando è richiesto il consenso, l’analisi si attiva solo dopo l’accettazione. Il sito può usare Google Analytics o Google Tag Manager; Google tratta i dati ricevuti secondo le proprie politiche. GeoSub conserva per impostazione predefinita gli eventi analitici grezzi di prima parte per un massimo di 180 giorni, poi li elimina o mantiene solo statistiche aggregate." },
  ...
  de: {
    ...
    sections: [
      ...
      { title: "Einwilligung, Anbieter und Speicherdauer", body: "Soweit eine Einwilligung erforderlich ist, startet die Analyse erst nach Zustimmung. Die Website kann Google Analytics oder Google Tag Manager einsetzen; Google verarbeitet empfangene Daten nach eigenen Richtlinien. GeoSub speichert eigene rohe Analyseereignisse standardmäßig höchstens 180 Tage und löscht sie anschließend oder bewahrt nur aggregierte Statistiken auf." },
  ...
  pt: {
    ...
    sections: [
      ...
      { title: "Consentimento, fornecedores e conservação", body: "Quando o consentimento é necessário, a análise só funciona depois da aceitação. O site pode utilizar Google Analytics ou Google Tag Manager; a Google trata os dados recebidos segundo as suas políticas. O GeoSub conserva, por predefinição, os eventos analíticos brutos próprios durante um máximo de 180 dias e depois elimina-os ou mantém apenas estatísticas agregadas." },
