...
export async function generateArticleSeoDraftAction(formData: FormData) {
  ...
  const seoDescription = truncateText(
    `${fallbackDescription} ${productNames.length > 0 ? `关联 ${productNames.slice(0, 3).join("、")} 等价格页。` : ""}`,
    155,
  );
  ...
  await prisma.auditLog.create({
    data: {
      ...
      note: "Generated article SEO draft from article content, tags, category, and relations.",
    },
  });

  revalidateArticlePaths(updated.slug);
  redirect(`/admin/articles/${updated.id}/edit?seoDrafted=1`);
}
