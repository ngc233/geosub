import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Data Sources - GeoSub",
  description: "GeoSub organizes pricing data from official subscription pages, public pricing pages, app store listings, regional pages, and manual review records.",
};

export default function EnglishTrustPage() {
  return (
    <main className="min-h-screen bg-[#faf8f2] px-5 py-16">
      <section className="mx-auto max-w-5xl">
        <p className="text-sm font-bold uppercase tracking-[0.2em] text-lime-600">
          Data Sources
        </p>

        <h1 className="mt-4 text-4xl font-black tracking-tight text-zinc-950 md:text-5xl">
          Data Sources
        </h1>

        <p className="mt-5 max-w-3xl text-lg leading-8 text-zinc-600">
          GeoSub organizes pricing data from official subscription pages, public pricing pages, app store listings, regional pages, and manual review records.
        </p>

        <div className="mt-10 rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm shadow-zinc-950/[0.03]">
          <h2 className="text-xl font-black text-zinc-950">
            Overview
          </h2>

          <p className="mt-4 text-sm leading-8 text-zinc-600">
            Prices, taxes, exchange rates, and platform policies may change over time. Final prices should always be verified on the official checkout page.
          </p>

          <p className="mt-6 text-sm leading-8 text-zinc-500">
            This is a basic English trust page. It can be expanded later with company details, analytics disclosure, advertising policy, affiliate policy, and data update rules.
          </p>
        </div>
      </section>
    </main>
  );
}